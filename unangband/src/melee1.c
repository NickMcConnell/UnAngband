	...
	switch (attack)
	{
		...
		case 96+3:
		{
			...
			if (dam) switch (method)
			{
				...
				case RBM_SCATTER: for (k = 0; k < (rlev / 15) + 1; k++) { scatter(&y, &x, m_ptr->fy, m_ptr->fx, 5, 0); (void)project(who, what, 0, m_ptr->fy, m_ptr->fx, y, x, dam, effect, FLG_MON_DIRECT, 0, 0); } break;
